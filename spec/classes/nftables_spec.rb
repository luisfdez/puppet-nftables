...
describe 'nftables' do
  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      ['ip', 'ip6', 'inet', 'arp', 'bridge', 'netdev'].each do |family|
      ...
      ['it', 'ip7', 'inet', 'arpa', 'brid', 'netdevs'].each do |family|
        context "with noflush_tables parameter set to invalid family #{family}" do
          let(:params) do
            {
              noflush_tables: ["#{family}-f2b-table"],
            }
            it { is_expected.to raise_error(Puppet::Error, %r{Invalid value ".*"\. Valid values are}) }
          end
        end
      end
    end
